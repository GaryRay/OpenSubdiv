...
//
// The original raydiff code in this file was contributed by Syoyo Fujita.
//
//
...
bool SolveLinearSystem22(const real A[2][2], const real B[2], real x[2]) {
...
//
void CalculatePlaneCoefficients(real3 &L0, real3 &L1, real3 &L2, const real3 v0,
  ...
  real3 Ln0 = cross(n, e21);
  Ln0.normalize();
  real3 Ln1 = cross(n, e02);
  Ln1.normalize();
  real3 Ln2 = cross(n, e10);
  Ln2.normalize();
  ...
  real w0_denom = dot(Ln0, v0) + d0;
  real w1_denom = dot(Ln1, v1) + d1;
  real w2_denom = dot(Ln2, v2) + d2;
...
void CalculateDifferencesForInterpolatedNormal(
    real3 &dNdx, /// [out]
    real3 &dNdy, /// [out]
    const real3 &P, const real3 &dPdx, const real3 &dPdy, const real3 &n0,
    const real3 &n1, const real3 &n2, const real3 &L0, const real3 &L1,
    const real3 &L2) {
  ...
  real dotL0P = dot(L0, P) + L0[3]; // homogeneous
  real dotL1P = dot(L1, P) + L1[3];
  real dotL2P = dot(L2, P) + L2[3];

  real3 n = dotL0P * n0 + dotL1P * n1 + dotL2P * n2;

  // printf("calc(n) = %f, %f, %f\n", n[0], n[1], n[2]);

  real dotL0dPdx = dot(L0, dPdx);
  real dotL1dPdx = dot(L1, dPdx);
  real dotL2dPdx = dot(L2, dPdx);

  real dotL0dPdy = dot(L0, dPdy);
  real dotL1dPdy = dot(L1, dPdy);
  real dotL2dPdy = dot(L2, dPdy);
  ...
  real dotnn = dot(n, n);
  real dotndndx = dot(n, dndx);
  real dotndndy = dot(n, dndy);

  // dN   (n.n) (dn/dx) - (n . (dn/dx)) n
  // -- = -------------------------------
  // dx             (n.n)^(3/2)

  dNdx = (dotnn * dndx - dotndndx * n) / (float)pow(dotnn, (3.0 / 2.0));
  dNdy = (dotnn * dndy - dotndndy * n) / (float)pow(dotnn, (3.0 / 2.0));

  // printf("dNdx = %f, %f, %f\n", dNdx[0], dNdx[1], dNdx[2]);
  // printf("dNdy = %f, %f, %f\n", dNdy[0], dNdy[1], dNdy[2]);
}

inline void FindBarycentricCoord(real &u, real &v, real &t, const real3 &v0,
                                 const real3 &v1, const real3 &v2,
...
bool ComputeDifferentialsForReflection(real3 &dDdx, real3 &dDdy, const Ray &ray,
                                       const real3 &P, const real3 &D,
                                       const real3 &N, const real3 &p0,
                                       const real3 &p1, const real3 &p2,
                                       const real3 &n0, const real3 &n1,
                                       const real3 &n2) {
  //
  // dP'   dP
  // --  = --
  ...
  if (!ray.hasDifferential) {
    ...
    CalculatePlaneCoefficients(L0, L1, L2, p0, p1, p2);
    ...
    CalculateDifferencesForInterpolatedNormal(dNdx, dNdy, P, dPdx, dPdy, n0, n1,
                                              n2, L0, L1, L2);
...
bool ComputeDifferentialsForRefraction(real3 &dDdx, real3 &dDdy, const Ray &ray,
                                       const real3 &P, const real3 &D,
                                       const real3 &N, const real3 &p0,
                                       const real3 &p1, const real3 &p2,
                                       const real3 &n0, const real3 &n1,
                                       const real3 &n2, real eta) {
  // P' = P
  // D' = eta D - mu N
  //
  // mu = eta ((D . N) - (D' . N))
...
bool ComputeDifferentialsForTransfer(real3 &dPdx, real3 &dPdy, real &dudx,
                                     real &dvdx, real &dudy, real &dvdy,
                                     ...
                                     const real3 &dPdv) ///< dP/dv (binormal)
{
  if (!ray.hasDifferential) {
    ...
    real d = -(dot(n, p));
    real3 rxv = ray.rx.P;
    real tx = -(dot(n, rxv) + d) / dot(n, ray.rx.D);
    real3 px = ray.rx.P + tx * ray.rx.D;

    real3 ryv = ray.ry.P;
    real ty = -(dot(n, ryv) + d) / dot(n, ray.ry.D);
...
bool ComputeTextureDifferentials(real3 &dTdx, ///< dT/dx [out]
                                 ...
                                 const real3 &tv1, const real3 &tv2,
