...
Shape * Shape::parseObj(char const * shapestr, Scheme shapescheme, int axis, bool parsemtl) {
    ...
    char * str=const_cast<char *>(shapestr), line[1024], buf[1024], usemtl=-1;
    bool done = false;
    while (not done) {
...
void Shape::addGroundPlane(float extent, float height) {
    int vindex = verts.size()/3;

    verts.push_back(-extent);
    verts.push_back(-extent);
