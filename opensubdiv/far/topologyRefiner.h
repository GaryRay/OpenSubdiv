...
public:
    ...
    //@{
    /// @name Child-to-parent relationships,
    /// Relationships between components in one level
    /// and the previous. Used for water-tight evaluation.
    ///
    Index GetChildFaceParentFace(int level, Index f) const {
        return _refinements[level]->getChildFaceParentFace(f);
    }
